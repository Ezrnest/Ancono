...
/**
 * calculator 只能计算单个formula的加减乘除和开方
 * @author lyc
 *
 */
	...
	public boolean canAdd(Formula f1,Formula f2){
		...
		if(f1.haveSameChar(f2)){
			if(this.state==0||(f1.isDecimal()&&f2.isDecimal())){
				return true;//都为或者为小数状态小数可以加
			}
			else if(f1.isDecimal()||f2.isDecimal()){
				return false;//有一个不为小数，一个为小数不可以加
			}
			else{
				if(f1.getRadical().compareTo(f2.getRadical())==0){
					return true;//两个都为分数，且根号部分相同可以加
				}
				else{
					return false;
				}
...
//			sum.setDecimal(true);
	...
	public Formula multiply(Formula f1,Formula f2){
		...
		if(this.state==0||f1.isDecimal()||f2.isDecimal()){//当计算器处于小数状态时，进行小数计算
...
//		result.sort();
	...
	/*
	 * 计算某个表达式的平方根：
	 * 当计算器状态处于 0 (小数)直接开根计算
	 * 当计算器状态处于 1 (分数)
	 * 		--f为小数表示或者f含有根号表达式 ：直接开根计算
	 * 		--f为分数表示且不含根号表达式：计算根号值
	 */
