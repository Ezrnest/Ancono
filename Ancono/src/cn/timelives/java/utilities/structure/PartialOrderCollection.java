 ...
 * in the partial order collection. The upper elements of element A are the elements that {@code e ≧ A }, and the downer elements 
 * are {@code A  ≧ e }.The union of the collection of upper elements and downer elements and the collection of <pre>{e| (! e ≧ A) && (! A ≧ e)}</pre>
 * should be the whole collection.The node A itself IS included in both upper elements and downer elements.
 * <P>
 * The direct upper elements of A are the elements that <pre>{e | e ≧ A && ([not exist](e' ∈ collection)(e ≧  e' ≧  A) ) }</pre>
...
public abstract class PartialOrderCollection<E> extends AbstractCollection<E>{
	...
	/**
	 * Return the elements in this collection that {e| (! e ≧ A) && (! A ≧ e)}, the union of {@code getUntouchedElements(e)}, {@code getUpperElements(e)} and 
