...
abstract class AbstractMatrix<T>(
    ...
    open fun subMatrix(rowStart: Int, rowEnd: Int, colStart: Int, colEnd: Int): AbstractMatrix<T> {
     ...
     *
     * The
     * inverse of a square matrix `A` is a matrix `B` such
    ...
    open fun solutionSpace(): VectorBasis<T> {
        TODO()
    }
...
abstract class Matrix<T>(
    ...
    override fun subMatrix(rowStart: Int, rowEnd: Int, colStart: Int, colEnd: Int): Matrix<T> {
        return SubMatrixView.subMatrixOf(this, rowStart, rowEnd, colStart, colEnd)
    }
    ...
    open fun toUpperTriangleWay(): TransformResult<T> {
        val m = MatrixImpl.copyOf(this, mathCalculator)
    ...
    open fun toIdentityWay(): TransformResult<T> {
        TODO()
    }
     ...
     * @return `(L,D)`
     */
    open fun decompCholeskyD(): Pair<Matrix<T>, Matrix<T>> {
        //Created by lyc at 2020-09-26 10:47
        TODO()
    }
...
//        return charMatrix(this, Polynomial.getCalculator(getMathCalculator()));
    ...
    open fun eigenvaluesAndVectors(equationSolver: EquationSolver<T, SVPEquation<T>>): List<Pair<T, Vector<T>>> {
        TODO()
    }
...
class AMatrix<T> internal constructor(
    ...
    override fun divideRow(r: Int, k: T, colStart: Int, colEnd: Int) {
        ...
        for (l in colStart until colEnd) {
            @Suppress("UNCHECKED_CAST")
            data[d + l] = mc.divide(k, data[d + l] as T)
        }
    }
    ...
    companion object {
        ...
        fun <T> of(row: Int, column: Int, mc: MathCalculator<T>, elements: List<T>): AMatrix<T> {
            require(elements.size == row * column && elements.isNotEmpty())
        ...
        fun <T> identity(n: Int, mc: MathCalculator<T>): AMatrix<T> {
            return diag(mc.zero, n, mc)
        }

        fun <T> fromVectors(vectors: List<AbstractVector<T>>, asRow: Boolean): AMatrix<T> {
...
internal object MatrixImpl {
    ...
    fun <T> solveLinear(expanded: MutableMatrix<T>, colSep: Int): Triple<Matrix<T>, VectorBasis<T>, Boolean> {
        ...
        val solvable = (r + 1 until expanded.row).all { i ->
    ...
    fun <T> inverse(m: AbstractMatrix<T>): AMatrix<T> {
        require(m.isSquare())
        val n = m.row
        val mc = m.mathCalculator
        ...
        return AMatrix.copyOfRange(expanded, 0, n, n, 2 * n)
    }
}
