...
public final class SimplificationStrategies {
    ...
    static final class SimplifyFraction extends SimpleStrategy {
        ...
        protected Node simplifyFraction(NodeFrac node, ExprCalculator mc) {
            ...
            if (t1 == Type.POLYNOMIAL && t2 == Type.POLYNOMIAL) {
                return null;
            }
...
//            list = mc.getMultinomialSimplifier().simplify(list);
    ...
    public static class Expand extends SimpleStrategy {
        ...
        /*
         * @see cn.ancono.math.numberModels.expression.SimpleStrategy#simplifyMultiply(cn.ancono.math.numberModels.expression.Node.Multiply, cn.ancono.math.numberModels.expression.ExprCalculator)
         */
        @Override
        protected Node simplifyMultiply(Multiply node, ExprCalculator mc) {
    ...
    public static abstract class SimplifyMultiplyStruct extends SimpleStrategy {
        ...
        /*
         * @see cn.ancono.math.numberModels.expression.SimStraImpl#simplifyMultiply(cn.ancono.math.numberModels.expression.Node.Multiply, cn.ancono.math.numberModels.expression.ExprCalculator)
         */
        @Override
        protected Node simplifyMultiply(Multiply node, ExprCalculator mc) {
...
//			/*
//			 * @see cn.ancono.math.numberModels.expression.SimpleStrategy#simplifyPolynomial(cn.ancono.math.numberModels.expression.Node.Poly, cn.ancono.math.numberModels.expression.ExprCalculator)
//			 */
//			@Override
...
//	}
     ...
     *     <li>exp(x,0) -> 1</li>
     *     <li>exp(x,1) -> x</li>
     *     <li>exp(exp(x,p1),p2) -> exp(x,p1*p2)</li>
     *     <li>exp(exp(p1),p2) -> exp(p1*p2)</li>
     *     <li>x * exp(x,y) * exp(x,z) -> exp(x,1+y+z)</li>
     * </ul>
     */
            ...
            protected Node simplifySFunction(SFunction node, ExprCalculator mc) {
                ...
                if (child.getType() == Type.MULTIPLY) {
                    ...
                    if (x != null) {
                        ...
                        return mc.simplify(Node.wrapNodeDF(ExprFunction.FUNCTION_NAME_EXP, x, mul));
                    }
                }
...
//                        return c1.cloneNode();
        ...
        list.add(new SimplifyMultiplyStruct(TAG_PRIMARY_SET, "x * exp(x,y) * exp(x,z) -> exp(x,1+y+z)") {
...
//			    return Node.isFunctionNode(n, "exp", 2);
        ...
        list.add(new SimplifyFunction(TAG_PRIMARY_SET, ExprFunction.FUNCTION_NAME_RECIPROCAL, "reciprocal(x) -> exp(x,-1)") {
            ...
            @Override
            protected Node simplifySFunction(SFunction node, ExprCalculator mc) {
                return mc.simplify(Node.wrapNodeDF(ExprFunction.FUNCTION_NAME_EXP, node.child.cloneNode(), Node.newPolyNode(Multinomial.NEGATIVE_ONE)));
            }
        });
        list.add(new ExpPossibleReduction());
        list.add(new ExpToFraction());
    ...
    static class ExpPossibleReduction extends SimpleStrategy {
        ...
        protected @Nullable Node simplifyMultiply(Multiply node, ExprCalculator mc) {
            ...
            for (Node n : children) {
                ...
                if (m.equals(re.getFirst())) {
                    continue;
                }
                m = re.getFirst();
                exp.setFirst(Node.newPolyNode(re.getSecond()));
     ...
     * </ul>
     */
    static class ExpToFraction extends SimpleStrategy {

        public ExpToFraction() {
            super(TAG_ALGEBRA_SET, CollectionSup.unmodifiableEnumSet(Type.D_FUNCTION), "exp",
                    "exp(x,-1) -> 1/x; " +
                            "exp(x/y,-1) -> y/x; ");
        }

        @Override
        protected @Nullable Node simplifyDFunction(DFunction node, ExprCalculator mc) {
            if (Boolean.parseBoolean(mc.getProperty(SimplificationStrategies.PROP_FRACTION_TO_EXP))) {
                return null;
            }
            ...
            if (c2.getType() == Type.POLYNOMIAL) {
                ...
                if (mulCal.isEqual(exp, Multinomial.NEGATIVE_ONE)) {
                    if (c1.getType() == Type.FRACTION) {
                        ...
                        //exp(x,-1) -> 1/x
                        return mc.simplify(Node.wrapNodeFraction(Node.newPolyNode(Multinomial.ONE), c1), 0);
                    }
                }
            }
            return null;
