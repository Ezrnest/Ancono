...
/**
 * A random process is a family of random variable `{X_n, n ≥ 0}` defined on a probability space
 ...
 * `Realization<R> := Function<Time, R>`, so a random process is actually a random variable
 * taking values as `Realization<R>`.
 * The random variable `X_n` can be taken using the function [GeneralRandomProcess.rvAt].
 *
 * Sampling from a random process is a bit different from the regular random variables,
 ...
 *
 */
interface RandomProcess<R> : DiscreteTimeRandomProcess<R>, SimpleRV<Realization<R>, Realization<R>> {
...
//    }
    ...
    /**
     * Shifts this random process with a stopping time, resulting in a random process of
     * > X_T, X_{T+1}, ...
     */
    fun shift(T: StoppingTime<R>): RandomProcess<R> {
        return ShiftedRandomProcess(this, T)
    }
 ...
 * using the values of the random process from `0` to `n`.
 *
...
open class StoppingTime<R>(
    ...
    companion object {
        ...
        fun <R> constant(process: RandomProcess<R>, n: Int): StoppingTime<R> {
...
class HittingTime<R>(process: RandomProcess<R>, val isHit: Predicate<R>)
...
class ShiftedRandomProcess<R>(val origin: RandomProcess<R>, val N: StoppingTime<R>) : RandomProcess<R> {
...
object RandomProcesses {
     ...
     */
    fun <T> randomWalk(start: T, X: RandomVariable<T>, mc: AbelGroupCal<T>): RandomWalk<T> {
        return RandomWalk(start, X, mc)
    }

    fun simpleRandomWalk(p: Double = 0.5, start: Int = 0): RandomWalk<Int> {
        val X = RandomVariables.bernoulli(p).map { it * 2 - 1 }
        return RandomWalk(start, X, Calculators.integer())
    }

    fun simpleSymmetricRW(start: Int = 0): RandomWalk<Int> {
...
//    val rw = RandomProcesses.randomWalk(20, X, Calculators.integer())
    val rw = RandomProcesses.simpleRandomWalk(18.0 / 38)
