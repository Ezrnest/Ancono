 ...
 * A n��m matrix has n lines and m rows. m,n=1,2,3,4,5...
 * <p>To create a Matrix , you can either use a two-dimension array as parameter or directly call
 * some prepared method.
 *
 * @author lyc
 * @deprecated this class is replaced by {@link cn.ancono.math.algebra.linearAlgebra.Matrix}
 */
@Deprecated
public abstract class Matrix {
    ...
    public Matrix subMatrix(int i1, int j1, int i2, int j2) {
        ...
        if (i1 < 0 || j1 < 0 || i2 >= line || j2 >= row || i1 >= i2 || j1 >= j2) {
            throw new IllegalArgumentException("Illegal Argument:" + line + "��" + row + ":(" + i1 + "," + j1 + ")-(" + i2 + "," + j2 + ")");
        }
        return null;//
    }
...
//				}
    ...
    static class SubMatrix extends Matrix {
        ...
        public int getNumber(int i, int j) {
            ...
            if (i < 0 || i >= line || j < 0 || j >= row) {
                throw new IndexOutOfBoundsException("Out of range:" + line + "��" + row + " " + i + "," + j);
            }
    ...
    public static Matrix valueOf(int[][] mat) {
        ...
        if (line < 1 || row < 1) {
            throw new IllegalArgumentException("Illegal size:" + line + "��" + row);
        }
    ...
    public static Matrix addMatrix(Matrix m1, Matrix m2) {
        if (m1.line != m2.line || m1.row != m2.row) {
            throw new IllegalArgumentException("Cannot add two matrix:" + m1.line + "��" + m1.row + " + " + m2.line + "��" + m2.row);
        }
        int line = m1.line;
        int row = m1.row;
        int[][] re = new int[line][row];
    ...
    public static Matrix minusMatrix(Matrix m1, Matrix m2) {
        if (m1.line != m2.line || m1.row != m2.row) {
            throw new IllegalArgumentException("Cannot minus two matrix:" + m1.line + "��" + m1.row + " - " + m2.line + "��" + m2.row);
        }
        int line = m1.line;
        int row = m1.row;
        int[][] re = new int[line][row];
    ...
    public static Matrix multiplyMatrix(Matrix m1, Matrix m2) {
        if (m1.row != m2.line) {
            throw new IllegalArgumentException("Cannot multiply two matrix:" + m1.line + "��" + m1.row + " �� " + m2.line + "��" + m2.row);
        }
        int n = m1.row;
        int line = m1.line;
        int row = m2.row;
    ...
    public static Matrix matrixPower(Matrix mat, int pow) {
        //do range check
        if (mat.line != mat.row)
            throw new IllegalArgumentException("Cannot calculate" + mat.line + "��" + mat.row);
        if (pow == 0)
