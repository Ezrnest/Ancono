 ...
 * The node in the {@linkplain DirectedGraph} is {@link DirectedGraph DirectedGraphNode<E>ode},this kind of node is more specific than general 
...
public abstract class DirectedGraph<E> implements Graph<E, DirectedGraphNode<E>>,Cloneable{
	 ...
	 * @see #getConnectToElement(DirectedGraph DirectedGraphNode<E>ode)
	 */
	 ...
	 * @see #getConnectTo(DirectedGraph DirectedGraphNode<E>ode)
	 */
	 ...
	 * @see #getConnectByElement(DirectedGraph DirectedGraphNode<E>ode)
	 */
	 ...
	 * @param node a node
	 * @return a collection of elements
	 * @see #getConnectBy(DirectedGraph DirectedGraphNode<E>ode)
	 */
	 ...
	 * This method is equal to {@link #isConnectTo(DirectedGraph DirectedGraphNode<E>ode, DirectedGraph DirectedGraphNode<E>ode)}.
	 ...
	 * This method is equal to {@link #isConnected(DirectedGraph DirectedGraphNode<E>ode, DirectedGraph DirectedGraphNode<E>ode)}.
	 ...
	 * This method is the parameter position transfered type of method {@link #isConnected(DirectedGraph DirectedGraphNode<E>ode, DirectedGraph DirectedGraphNode<E>ode)}.
	 * The call of {@code isConnectBy(n1,n2)} is equal to {@code isConnectTo(n2,n1)}.The method is created just for a clear 
	 * view of code.
	 * @param n1 a node 
	 * @param n2 another node
	 * @return {@code true} if n1 refers to n2 , otherwise {@code false}
	...
	/**
	 * Connect node n1 to node n2.This method is equal to {@link #connect DirectedGraphNode<E>ode(DirectedGraph DirectedGraphNode<E>ode, DirectedGraph DirectedGraphNode<E>ode)}.
	 * The method is created just for a clear view of code.
	 * <p>{@literal n1 -> n2}
	 ...
	 * transfered type of method {@link #connectTo(DirectedGraph DirectedGraphNode<E>ode, DirectedGraph DirectedGraphNode<E>ode)}.
	 ...
	 * no disconnecting method to this two node after that.This method is equal to {@link #connectTo(DirectedGraph DirectedGraphNode<E>ode, DirectedGraph DirectedGraphNode<E>ode)}.
	 */
	@Override
	...
	/**
	 * Connect all the nodes from nodes,which means for each pair of node (n1,n2), isConnected(n1,n2) will return true. DirectedGraphNode<E>otice 
	 ...
	 *  DirectedGraphNode<E>otice that both directions of connection will be removed and self-connection will be removed as well.
	 */
	...
	/**
	 * Compute a sub graph of this directed graph.The sub graph contains all the nodes in {@code nodes} and all the connection 
	...
	/**
	 * Returns a new sub graph of this directed graph.The sub graph is a graph that only contains the given node and all its 
	 * connect-to nodes,and the connections in the new graph are only these connections , which means in the new graph , {@code isConnectTo(n1,n2)==true} 
	 * if and only if {@code n1 == node}.Even if there may exist nodes that have both-direction connection with {@code node}, the 
	...
	/**
	 * Returns a new sub graph of this directed graph.The sub graph is a graph that only contains the given node and all its 
	 * connect-by nodes,and the connections in the new graph are only these connections , which means in the new graph , {@code isConnectBy(n1,n2)==true} 
	 * if and only if {@code n1 == node}.Even if there may exist nodes that have both-direction connection with {@code node}, the 
	 * connections in the new graph will only remains one direction.   
	 * @param node a node in this graph
	 * @return a sub graph
