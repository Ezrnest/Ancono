...
/**
 * PartialOrderCollection is a collection of partial ordered elements.The collection provides more methods than add or
 * remove,such as
 * {@link #getUpperElements(Object)} (Object)} or {@link #getDownerElements(Object)} (Object)}.
 * This collection will give out proper returning values for these methods.
 * <p>
 * <h1>Upper and Downer</h1>
 * Upper elements (parent elements) and downer elements (child elements) are basic expression of the relationship of the
 * elements
 * in the partial order collection. The upper elements of element A are the elements that {@code e ≧ A }, and the downer
 * elements
 * are {@code A  ≧ e }.The union of the collection of upper elements and downer elements and the collection of <pre>{e|
 * (! e ≧ A) && (! A ≧ e)}</pre>
 * should be the whole collection.The node A itself IS included in both upper elements and downer elements.
 * <p>
 * The direct upper elements of A are the elements that <pre>{e | e ≧ A && ([not exist](e' ∈ collection)(e ≧  e' ≧  A) )
 * }</pre>
 * Which means there aren't any elements in the collection to be between the direct upper element and the element A.The
 * direct downer elements
 * is similar , <pre>{e | A ≧ e && ([not exist](e' ∈ collection)(A ≧  e' ≧  e) ) }</pre>
 * The direct upper elements and downer elements are usually needed in some mathematical calculation or graph.See
 * {@linkplain cn.timelives.java.utilities.structure.DirectedGraph} for some
...
public abstract class PartialOrderCollection<E> extends AbstractCollection<E> {
    ...
    /**
     * This method will search this collection for an element {@code equal} that {@code
     * comparator.isEqual(equal,ele)==true}.
     * If there is such an element,then the element will be returned, otherwise, {@code null} will be returned. If an
     * element
     * that is added to this collection formally is used as a parameter,then the identity object will be returned({@code
     * ele == containsEqualElement(ele)}).
     *
    ...
    /**
     * Remove the element {@code o} in this collection, this method will only remove the element if {@code
     * contains(o)==true},
     * notice that this method will not remove the element that {@code e.equals(o)==false} while {@code
     * comparator.isEqual(e,o) == true}.
     */
    @Override
    public abstract boolean remove(Object o);

    /**
     * Remove the element e that {@code comparator.isEqual(e,ele)==true}.These method will only remove one such
     * element,the order
     ...
     * getUntouchedElements(e)}, {@code getUpperElements(e)} and
